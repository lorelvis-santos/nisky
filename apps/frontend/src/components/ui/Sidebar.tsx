    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 shrink-0 flex-col border-r border-outline-variant bg-surface transition-transform duration-150 md:static md:z-auto md:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="border-b border-outline-variant px-container-padding py-3">
          ...
          <p className="truncate font-data-mono text-data-mono text-on-surface-variant">{user?.email}</p>
        </div>
        <nav className="flex flex-1 flex-col gap-element-gap-xs overflow-y-auto py-element-gap-md">
          {primaryItems.map((item) => <NavItem {...item} key={item.href} onNavigate={onClose} />)}
        </nav>
        <div className="border-t border-outline-variant py-element-gap-xs">
          {secondaryItems.map((item) => <NavItem {...item} key={item.href} onNavigate={onClose} />)}
        </div>
