    ...
    <Dialog open onOpenChange={(nextOpen) => { if (!nextOpen) onClose(); }}>
      <DialogContent className="max-w-md overflow-hidden rounded-lg border-outline-variant bg-surface p-0" showCloseButton={false}>
        ...
        <DialogFooter className="mt-0 flex-row justify-end border-t-0 bg-surface-container-low px-5 py-4">
          <button className="min-h-11 rounded-lg border border-outline-variant px-4 py-2 font-body-md text-body-md text-on-surface-variant hover:bg-surface-container-low disabled:opacity-50" disabled={loading} onClick={onClose} type="button">
            {cancelLabel}
          </button>
          <button
            className={`${danger ? "bg-error text-error-foreground hover:bg-error/90" : "bg-primary-container text-on-primary hover:bg-primary"} min-h-11 rounded-lg px-4 py-2 font-body-md text-body-md disabled:opacity-50`}
            disabled={loading}
            onClick={onConfirm}
            type="button"
          >
