    ...
    <div className="space-y-3">
      ...
      {isLoading ? (
        <p className="flex items-center gap-2 font-body-sm text-body-sm text-on-surface-variant"><Loader2 className="animate-spin" size={16} /> Cargando feedback…</p>
         ...
         <p className="rounded-lg border border-outline-variant bg-surface p-container-padding font-body-sm text-body-sm text-on-surface-variant shadow-cadence-1">
          No hay feedback con estos filtros.
        </p>
      ) : (
         <ul className="divide-y divide-outline-variant rounded-lg border border-outline-variant bg-surface shadow-cadence-1">
          {items.map((item) => (
            <li className="flex flex-wrap items-start gap-3 p-container-padding" key={item.id}>
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 flex flex-wrap items-center gap-2 font-body-sm text-body-sm text-on-surface-variant">
                   <span className="rounded-full border border-outline-variant px-1.5 py-0.5 font-label-caps text-label-caps uppercase">{CATEGORY_LABEL[item.category]}</span>
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <select
                  aria-label="Cambiar estado"
                   className="field min-h-11 h-auto w-auto text-xs"
                  onChange={(e) =>
