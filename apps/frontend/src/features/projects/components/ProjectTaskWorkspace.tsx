...
function ProjectTaskRow({ task, onOpen, onToggle, onStartPomodoro }: { task: Task; onOpen: () => void; onToggle: () => void; onStartPomodoro: () => void }) {
  ...
  return (
    <article className={cn("first:rounded-t-lg last:rounded-b-lg grid grid-cols-[44px_minmax(0,1fr)] gap-2 px-2 py-2 transition-colors hover:bg-[#fafaf8] sm:px-3 md:grid-cols-[44px_minmax(0,1fr)_7.25rem_6.5rem_7.5rem_7rem_2.75rem] md:items-center lg:px-4", completed && "bg-[#fdfdfb]")}>
      ...
      <div className="flex min-w-0 items-start gap-2 md:hidden">
         <button className="min-w-0 flex-1 rounded-md text-left" onClick={onOpen} type="button">
           ...
           <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-[#858d91]">
            ...
            {task.dueDate && <span className={overdue ? "font-semibold text-[#c73b52]" : undefined}>{formatDateTime(task.dueDate)}</span>}
      ...
      <span className="hidden md:inline-flex"><PriorityChip priority={task.priority} /></span>
       <span className={cn("hidden items-center gap-1 text-[11px] md:flex", overdue ? "font-semibold text-[#c73b52]" : "text-[#5f6872]")}>{task.dueDate ? <>{formatDateTime(task.dueDate)}</> : <span className="text-[#9aa2a5]">Sin fecha</span>}</span>
